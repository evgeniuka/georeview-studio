...
TRANSIT_ACCESS_ANALYZER_VERSION = "transit_access_analyzer_v001"
DEFAULT_TRANSIT_WORKSPACE_ID = "transit_stop_walk_access_kfar_saba_v001"
...
class TransitAccessAnalyzer:
    ...
    def ensure_workspace(
        ...
        workspace_id: str = DEFAULT_TRANSIT_WORKSPACE_ID,
    ) -> dict:
        ...
        for risk in risk_rows:
            if risk.get("generator_type") != "bus_stop":
                continue
            network = network_by_generator.get(risk.get("generator_id"), {})
            rows.append(self.transit_row(risk, network))
        rows.sort(
            key=lambda row: (
                -parse_int(row.get("transit_review_priority_score")),
        ...
        results_path = output_tables / "transit_stop_access_results.csv"
        top20_path = output_tables / "transit_stop_access_top20.csv"
        summary_path = output_reports / "transit_access_summary.json"
        markdown_path = output_reports / "transit_access_summary.md"
        ...
        manifest = {
            ...
            "analyzer": TRANSIT_ACCESS_ANALYZER_VERSION,
            "source_gis_modified": False,
            "tables": [
                {"table": "transit_stop_access_results", "file": str(results_path), "rows": len(rows)},
                {"table": "transit_stop_access_top20", "file": str(top20_path), "rows": min(20, len(rows))},
            ],
            "reports": {
                "transit_access_summary": str(summary_path),
                "transit_access_markdown": str(markdown_path),
            },
        }
        write_json(manifest_path, manifest)
        readme_path.write_text(
            "# Transit Stop Walk Access Workspace\n\n"
            f"Derived from `{base_workspace_id}` by GeoReview Studio.\n\n"
            "This workspace summarizes infrastructure review indicators around mapped bus stops. "
            "It does not predict crashes and does not prove real-world access conditions.\n",
            encoding="utf-8",
        )
        return {
            "ok": True,
    ...
    def transit_row(self, risk: dict, network: dict) -> dict:
        ...
        return {
            ...
            "transit_review_priority_score": max(base_score, route_score),
            "transit_access_flags": json.dumps(flags, ensure_ascii=False),
    ...
    @staticmethod
    def transit_flags(risk: dict, network: dict) -> list[str]:
        flags: list[str] = []
    ...
    def summary(self, rows: list[dict], base_workspace_id: str, workspace_id: str) -> dict:
        ...
        for row in rows:
            for flag in read_json_list(row.get("transit_access_flags")):
        ...
        return {
            ...
            "counts": {
                "transit_stops": len(rows),
                "stops_without_mapped_crossing_within_150m": sum(1 for row in rows if str(row.get("crossing_within_150m")).lower() != "true"),
                "stops_near_major_road_150m": sum(1 for row in rows if boolish(row.get("major_road_within_150m"))),
                "stops_route_crossing_over_250m": sum(1 for row in rows if parse_number(row.get("route_nearest_crossing_m")) > 250),
            },
    ...
    def summary_markdown(self, summary: dict, rows: list[dict]) -> str:
        ...
        lines = [
            "# Transit Stop Walk Access Summary",
            "",
            ...
            "",
            "This profile summarizes infrastructure review indicators around mapped bus stops. It is not a crash prediction.",
            "",
            "## Counts",
            "",
            "| Metric | Value |",
        ...
        lines.extend([
            ...
            "## Top Stops",
            "",
            "| Stop | Name | Route crossing m | Score | Flags |",
            "| --- | --- | ---: | ---: | --- |",
        ])
        for row in rows:
            flags = ", ".join(read_json_list(row.get("transit_access_flags"))[:3])
            lines.append(
                f"| {row.get('transit_stop_id')} | {str(row.get('name') or '').replace('|', ' ')} | "
                f"{row.get('route_nearest_crossing_m')} | {row.get('transit_review_priority_score')} | {flags} |"
            )
        lines.extend([
            "",
            "## Limitations",
    ...
    def list_workspaces(self) -> list[dict]:
        ...
        for manifest_path in sorted(self.workspaces_dir.glob("*/manifest.json"), key=lambda item: item.stat().st_mtime, reverse=True):
            ...
            rows.append({
                ...
                "transit_stops": summary.get("counts", {}).get("transit_stops"),
    ...
    def summary_for_workspace(self, workspace_id: str) -> dict:
        ...
        summary_path = workspace_dir / "reports" / "transit_access_summary.json"
    ...
    def results(self, workspace_id: str, limit: int = 50) -> list[dict] | dict:
        workspace_dir = self.workspaces_dir / workspace_id
        results_path = workspace_dir / "tables" / "transit_stop_access_results.csv"
    ...
    def output_file(self, workspace_id: str, output_id: str) -> dict:
        workspace_dir = self.workspaces_dir / workspace_id
        candidates = {
            "transit_stop_access_results": workspace_dir / "tables" / "transit_stop_access_results.csv",
            "transit_stop_access_top20": workspace_dir / "tables" / "transit_stop_access_top20.csv",
            "transit_access_summary": workspace_dir / "reports" / "transit_access_summary.json",
            "transit_access_markdown": workspace_dir / "reports" / "transit_access_summary.md",
        }
